...
def _actionable(row):
    action = str(row.get("action_at_forecast") or "").upper()
    return action not in {"WAIT", "NO_TRADE", "RESEARCH_ONLY"}
...
def _consensus_reliable(row):
    if row.get("market_consensus_timestamp_safe") is not True:
        return False
    if row.get("market_consensus_reliable") is not True:
        return False
    try:
...
def _status(profile, dev, val, dev_base, val_base):
    ...
    dev_base_ev = dev.get("base_after_cost_expectancy_pct")
    val_base_ev = val.get("base_after_cost_expectancy_pct")
    if dev_base_ev is not None and val_base_ev is not None and float(dev_base_ev) <= 0 and float(val_base_ev) <= 0:
        return "RESTRICTIVE_WAIT_CANDIDATE"
    robust = (
        dev_lift >= MIN_PRECISION_LIFT
        and val_lift >= MIN_PRECISION_LIFT
        and float(dev.get("worst_case_after_cost_expectancy_pct") or -1e9) > 0.0
        and float(val.get("worst_case_after_cost_expectancy_pct") or -1e9) > 0.0
    )
    if robust:
...
def _supporting_readiness(regime_strategy, ensemble_diversity, microstructure_veto):
    ...
    diversity_pairs = len((ensemble_diversity or {}).get("pairs") or [])
    micro_samples = int((microstructure_veto or {}).get("samples_with_microstructure") or 0)
    return {
        "regime_router_candidates": regime_candidates,
        "regime_restrictive_wait_candidates": regime_waits,
        "ensemble_diversity_pairs_measured": diversity_pairs,
        "prospective_microstructure_samples": micro_samples,
...
def build_meta_signal_trust(rows, *, regime_strategy=None, ensemble_diversity=None, microstructure_veto=None, cost_pct=ROUND_TRIP_COST_PCT):
    ...
    for horizon in ("24h", "7d"):
        ...
        for profile in profiles:
            ...
            reports.append({
                ...
                "development": dev,
                "validation": val,
                "development_precision_lift_vs_actionable": _precision_lift(dev, dev_base),
                "validation_precision_lift_vs_actionable": _precision_lift(val, val_base),
    ...
    return {
        ...
        "a_plus_definition": "High-confidence + timestamp-safe multi-source market consensus; >=2pp precision lift in development and validation; positive expectancy through 3x fixed cost stress. Research candidate only.",
