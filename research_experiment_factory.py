...
def _contract(item: dict, hypothesis: str) -> dict:
    contract = {
        ...
        "predicted_mechanism": str(item.get("predicted_mechanism") or "A diagnosed independent economic-loss or error condition may support a restrictive filter or challenger."),
        "target_horizon": str(item.get("target_horizon") or "both"),
        "expected_signal_quality_effect": str(item.get("expected_signal_quality_effect") or "Increase after-cost expectancy first; improve genuine BUY/SELL precision or WAIT quality second."),
...
def build_experiment_queue(diagnostics: dict, memory: dict | None = None, *, limit: int = MAX_EXPERIMENTS) -> dict:
    ...
    eligible_priorities = [
        item for item in (priorities or [])
        if isinstance(item, dict) and item.get("requires_new_validation") is True
    ]
    maximum_economic_harm = max(
        (max(0.0, _finite(item.get("economic_harm_score_pct"))) for item in eligible_priorities),
        default=0.0,
    )
    experiments = []
    for item in eligible_priorities:
        ...
        economic_harm = max(0.0, _finite(item.get("economic_harm_score_pct")))
        if maximum_economic_harm > 0:
            profitability_impact = max(0.05, min(1.0, economic_harm / maximum_economic_harm))
        else:
            # Keep bounded exploratory research possible when after-cost evidence is not yet
            # available, but make it lower priority than a measured economic-loss condition.
            profitability_impact = 0.05
        factors = {
            "expected_incremental_after_cost_profitability_impact": round(profitability_impact, 4),
        ...
        information_priority = priority_score(
            expected_incremental_after_cost_profitability_impact=factors["expected_incremental_after_cost_profitability_impact"],
            expected_information_falsification_value=factors["expected_information_falsification_value"],
        ...
        experiments.append({
            ...
            **contract,
            ...
            "source_average_after_cost_return_pct": item.get("average_after_cost_return_pct"),
            "source_economic_harm_score_pct": economic_harm,
    ...
    experiments.sort(
        key=lambda row: (
            ...
            -float((row.get("priority_factors") or {}).get("expected_genuine_signal_quality_impact", 0)),
            -row["source_independent_samples"],
            row["experiment_id"],
        )
    )
    ...
    return {
        ...
        "trade_authority": False,
        "promotion_authority": False,
        "policy": "Experiment specs are hypotheses only. Ranking uses expected incremental after-cost profitability impact x falsification value x actionable-evidence probability / bounded compute cost; genuine forward signal-quality impact is a secondary tiebreaker. Every result still requires canonical validation.",
    }
