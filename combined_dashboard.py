...
def _calibration(row):
    ...
    if not isinstance(c, dict):
        return {"ready": False, "precision": None, "samples": 0, "minimum": 30}
    try:
    ...
    try:
        ...
        precision = None
    return {"ready": ready, "precision": precision, "samples": samples, "minimum": minimum}
...
def combined_dashboard_page(request: Request, horizon: str = "all"):
    ...
    for rank, row in enumerate(rows, 1):
        ...
        if conf is not None:
            validation_sub = f"N={cal['samples']} verified"
        else:
            validation_sub = f"N={cal['samples']}/{cal['minimum']} forward samples"
...
<div class="topbar"><div class="brand"><div class="logo">▥</div><div><h1>Crypto Signals <span class="live">● LIVE</span></h1><div class="subtitle">Multi-timeframe · AI research · unified opportunities · 24/7 · refreshes every 60s</div></div></div><input id="search" class="search" placeholder="Search any coin (e.g. BTC, XRP, PONS...)" autocomplete="off"></div>
<div class="cards">
...
<div class="card"><div class="label">STRONG EVIDENCE</div><div class="value">{strong_evidence}</div><div class="hint">evidence score ≥80/100</div></div>
...
<div class="card"><div class="label">2×+ OPPORTUNITIES</div><div class="value">{strong_moves}</div><div class="hint">100%+ expected move</div></div>
<div class="card"><div class="label">ACTIONABLE</div><div class="value">{active}</div><div class="hint">passed actual TRADE gate</div></div>
...
<div class="panel"><div class="panel-head"><div class="panel-title">Live Crypto Signals</div><div class="scan"><b>●</b> Real trading-system data · one strongest opportunity per asset</div></div>
<div class="tablewrap"><table><thead><tr><th>#</th><th>ASSET</th><th>DIRECTION</th><th>SETUP</th><th>VALIDATION</th><th>EXPECTED MOVE</th><th>EST. DURATION</th><th>ENTRY</th><th>TARGET</th><th>STOP</th><th>R:R</th><th>STATUS</th></tr></thead><tbody>{table_body}</tbody></table></div>
<div class="foot"><span>Evidence measures current signal strength. Validation is separate and appears only after enough independent forward samples.</span><span>No performance numbers are invented to fill the dashboard.</span></div></div>
</div><script>
...
function apply(){{const q=(search.value||'').trim().toLowerCase();rows.forEach(r=>{{const ms=!q||r.dataset.search.includes(q);let mf=true;if(active==='evidence')mf=parseFloat(r.dataset.evidence)>=80;if(active==='validated')mf=parseFloat(r.dataset.confidence)>0;if(active==='strong')mf=parseFloat(r.dataset.move)>=100;if(active==='active')mf=r.dataset.status==='active';if(active==='long')mf=r.dataset.side==='long';if(active==='short')mf=r.dataset.side==='short';r.style.display=(ms&&mf)?'':'none';}})}}
