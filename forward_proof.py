...
MIN_FORWARD_SAMPLES = {"24h": 20, "7d": 12}
...
def assess_forward_proof(identity, horizon, resolved_rows):
    """Evaluate exact-fingerprint genuine forward evidence.

    The result is restrictive only. It never creates promotion authority; the
    signed promotion manifest, Strategy Registry, and Production Risk approvals
    ...
    if not fingerprint or minimum is None:
        return {
            "passed": False,
            "status": "FORWARD_PROOF_UNAVAILABLE",
            "reason": "missing_exact_identity_or_supported_horizon",
            "independent_samples": 0,
        }

    matching = [
        row for row in (resolved_rows or [])
        if row.get("horizon") == horizon
        and _fingerprint(row) == fingerprint
    ...
    successes = sum(value > 0 for value in net_returns)
    sample_count = len(net_returns)
    precision = successes / sample_count if sample_count else 0.0
    lower = wilson_lower_bound(successes, sample_count)
    expectancy = sum(net_returns) / sample_count if sample_count else None
    drawdown = _max_drawdown_pct(net_returns) if net_returns else None

    deterioration_rows = []
    for (row, _), net_return in zip(selected, net_returns):
        copied = dict(row)
        copied["correct"] = net_return > 0
        deterioration_rows.append(copied)
    deterioration = deterioration_assessment(deterioration_rows)

    sample_ready = sample_count >= minimum
    positive_expectancy = expectancy is not None and expectancy > 0
    precision_pass = sample_ready and lower >= MIN_FORWARD_PRECISION_LOWER
    drawdown_pass = drawdown is not None and drawdown <= MAX_FORWARD_DRAWDOWN_PCT
    deterioration_pass = not deterioration.get("deteriorating", False)
    passed = bool(sample_ready and positive_expectancy and precision_pass and drawdown_pass and deterioration_pass)

    if not sample_ready:
        reason = "insufficient_independent_forward_samples"
    elif not positive_expectancy:
        reason = "non_positive_after_cost_forward_expectancy"
    elif not precision_pass:
        reason = "forward_precision_confidence_too_weak"
    elif not drawdown_pass:
        reason = "forward_drawdown_too_high"
    elif not deterioration_pass:
        reason = "recent_forward_deterioration"
    else:
    ...
    return {
        ...
        "after_cost_expectancy_pct": round(expectancy, 6) if expectancy is not None else None,
        "max_forward_drawdown_pct": round(drawdown, 4) if drawdown is not None else None,
        "modeled_round_trip_cost_bps": BACKTEST_COST_BPS * FORWARD_COST_MULTIPLIER,
