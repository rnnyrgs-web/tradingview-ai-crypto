...
def test_long_entry_uses_two_leg_usd_bridge_only_with_visible_depth(monkeypatch):
    books = {
        ("FET", "USDT"): RuntimeError("kraken_pair_unavailable"),
        ("USDT", "USD"): (
            "USDTUSD",
        ...
        ("FET", "USD"): (
            ...
            {
                "bids": [[0.1800, 50000]],
                "asks": [[0.1810, 50000], [0.1820, 50000]],
            },
        ),
    }
    ...
    assert result.source_count == 2
    assert result.raw_vwap is not None and result.raw_vwap > 0
    assert result.fill_price is not None and result.fill_price > result.raw_vwap
    assert result.fee_bps == 100.0
    assert result.levels_used >= 2
    assert result.supported_notional is not None and result.supported_notional >= 7200.0
    assert math.isfinite(result.worst_slippage_bps)
...
def test_long_usd_bridge_fails_closed_when_quote_leg_has_insufficient_depth(monkeypatch):
    books = {
        ("FET", "USDT"): RuntimeError("kraken_pair_unavailable"),
        ("USDT", "USD"): (
            "USDTUSD",
    ...
    assert result.executable is False
    assert result.reason == "insufficient_kraken_visible_depth_usd_bridge_leg1"
    assert result.source_count == 2
