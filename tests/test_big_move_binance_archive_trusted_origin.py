...
def _local_only_binance_record(tmp_path) -> dict:
    # Deliberately not provider-authenticated. The ZIP signature only makes this a
    # syntactically plausible archive body; the bytes were never fetched from Binance.
...
def _canonical_but_unattested_c101h_receipt(archive_raw: bytes) -> dict:
    """Create a real C101-H receipt shape without any external attestation authority."""
    ...
    # This mapping looks like the trusted workflow context but is caller-supplied here.
    # canonical_receipt intentionally does not prove GitHub artifact attestation itself.
    github_context = {
...
def test_canonical_binance_archive_rejects_canonical_but_unattested_receipt(tmp_path):
    """Even a canonical receipt is caller data until an out-of-band boundary verifies it."""
    record = _local_only_binance_record(tmp_path)
