...
def test_every_worker_class_has_explicit_signal_development_path():
    ...
    required = {
        "python_research_workers", "cross_asset_rank_24h", "cross_asset_rank_7d",
        "learning_diagnostics", "experiment_factory", "heavy_experiment_scheduler",
        "validation_workers", "autonomous_cloud_specialist", "lead_coordination",
...
def test_ai_specialists_and_lead_have_explicit_signal_paths():
    bindings = json.loads(Path("orchestration/signal_worker_bindings.json").read_text(encoding="utf-8"))
    required = {
        "autonomous-cloud-data-market", "quant-research", "signal-accuracy", "data-market",
        "regime-selection", "execution-microstructure", "production-risk", "testing-security",
        "lead-integrator",
    }
...
def test_task_contract_requires_falsifiable_measurable_fields():
    diagnostics = {"research_priorities": [{
        "dimension": "direction", "group": "BUY",
        "research_question": "Can a restrictive condition reduce BUY false positives?",
        "requires_new_validation": True, "samples": 30, "independent_samples": 20,
        "wrong_rate": 0.4, "target_horizon": "24h",
    }]}
    ...
    bad = dict(experiment); bad.pop("falsification_criteria")
    with pytest.raises(ObjectiveError):
