...
def test_quant_role_becomes_eligible_after_data_task_completes():
    coordination = copy.deepcopy(_coord())
    data_task = _ready_discovery_task_for_role("data-market", coordination)
    data_task["status"] = "DONE"
    expected = _ready_discovery_task_for_role("quant-research", coordination)
    task = highest_ready_task(_config(), coordination)
    assert task == expected
    assert task["owner"] == "quant-research"
    assert task["fingerprint_id"] == data_task["fingerprint_id"]
