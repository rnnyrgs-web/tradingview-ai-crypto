...
def test_atomic_claim_branch_naming_matches_shared_convention():
    assert safe_branch("testing-security", "COORD-DISC-TEST-002") == "auto/testing-security/coord-disc-test-002"
...
def test_plan_selects_the_owned_ready_task_and_no_other_role():
    ...
    assert decision.task_id == "COORD-DISC-TEST-003"
    assert decision.branch == "auto/testing-security/coord-disc-test-003"
...
def test_no_retry_runner_does_not_multiply_reservation_for_nonexistent_retries():
    config = _config()
