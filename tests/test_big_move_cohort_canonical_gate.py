...
def test_authoritative_gate_keeps_outcomes_sealed_while_evidence_bindings_are_open():
    ...
    assert set(result["source_value_binding_blockers"]) == set(SOURCE_VALUE_BINDING_BLOCKERS)
    assert all(
        f"DERIVED_OUTPUT_NOT_DETERMINISTICALLY_BOUND:{field}" in result["blockers"]
        for field in DERIVED_OUTPUT_BINDING_BLOCKERS
    )
    assert all(blocker in result["blockers"] for blocker in SOURCE_VALUE_BINDING_BLOCKERS)
