...
REQUIRED_TASK_FIELDS = {
    "hypothesis", "predicted_mechanism", "target_horizon", "expected_signal_quality_effect",
    "evidence_needed", "falsification_criteria", "chronological_oos_requirements",
    "realistic_cost_treatment", "independent_sample_requirements", "status", "result",
    "evidence_conclusion",
}
...
def validate_objective(payload: dict) -> None:
    ...
    required_false = (
        ...
        "reset_paper_evidence", "fabricate_historical_data", "weaken_validation_gates",
    ...
    if set(payload.get("task_contract_required_fields") or []) != REQUIRED_TASK_FIELDS:
...
def validate_task_contract(task: dict) -> dict:
    ...
    for key in ("evidence_needed", "falsification_criteria", "chronological_oos_requirements", "realistic_cost_treatment", "independent_sample_requirements"):
...
def objective_reference(worker_name: str, worker_class: str) -> dict:
    ...
    return {
        "objective_id": objective["objective_id"], "primary_mission": objective["primary_mission"],
        "optimization_target": objective["optimization_target"], "worker_name": worker_name,
        "worker_class": worker_class, "purpose": registry[worker_class]["purpose"],
        "signal_path": registry[worker_class]["signal_path"], "trade_authority": False,
...
def _after_cost(rows):
    ...
    for row in rows:
        try:
            value = float(row.get("after_cost_return_pct"))
...
def build_signal_quality_scorecard(rows: list[dict]) -> dict:
    objective = load_objective(); horizons = {}
    for horizon in ("24h", "7d"):
        ...
        stages = defaultdict(list); regimes = defaultdict(list); liquidities = defaultdict(list)
        ...
        horizons[horizon] = {
            "buy": _precision(independent, "BUY"), "sell": _precision(independent, "SELL"),
            "wait_quality": {"precision": round(wait_correct / len(wait_rows), 4) if wait_rows else None, "independent_observations": len(wait_rows)},
            "after_cost": _after_cost(independent), "independent_observation_count": len(independent),
    ...
    return {
        "objective_id": objective["objective_id"], "optimization_target": objective["optimization_target"],
        "evidence_policy": objective["scorecard"]["evidence_policy"], "horizons": horizons,
