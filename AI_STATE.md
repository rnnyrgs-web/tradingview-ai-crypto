...
PR #141 completed the next bounded ACC-002 observability step. Exact head `9770af1f95b99e458cdf29e40f55627cc0e0e8b5` passed Security and Reliability run `34358842703` (#1052) with unit tests, dependency audit, static security scan and secret scan all green, then was squash-merged as `149c11bf8e28f9cdf438405549e8a43478094102`. The ACC-002 producer now exposes only aggregate `failure_type_counts` alongside the existing bounded failed-symbol count, and the coordinator validates and logs those aggregate counts without symbol identities or raw samples. Regression tests cover `InsufficientHistory` aggregation, identity privacy, malformed-count filtering and legacy sanitized compatibility. No liquidity threshold, history requirement, OOS gate, strategy behavior, data source, worker allocation, paper state, broker state, promotion authority or production signal behavior changed.

## ACCURACY / RESEARCH PROGRAM
- ACC-001 MARKET / EXECUTION REALISM — COMPLETE.
- ACC-002 CROSS-ASSET RANK RESEARCH — IN PROGRESS and currently FAIL-CLOSED before untouched OOS. Corrected bounded evidence shows the predeclared Top-15 subset only: a representative 24h run resolved 24/30 requested assets with 6 failed histories, while the first genuine corrected 7d run resolved 16/30 with 14 failed histories. Both reported `supported_liquidity_subsets=[15]`, canonical minimum subset coverage remains 0.80, and untouched OOS remained unopened because two supported subsets are required. There is no ACC-002 pass/fail, survivorship pass/fail, accuracy, or profitability conclusion yet. PR #141 now permits future runs to classify the aggregate failure causes without exposing symbol identities. The beta-neutral residual-momentum challenger from PR #131 must not bypass this blocker or any canonical gate.
...
PR #135 is an observability-only follow-up to a genuine blocker, not a strategy change. PR #136 is an observability-only correction to PR #135. PR #141 is the bounded cause-classification follow-up. Regression coverage models the actual bounded producer summary shape, verifies top-level supported-subset/failed-count handling and aggregate failure-type handling, retains sanitized legacy compatibility, and confirms the threshold is the unchanged canonical 80% constant rather than a loosened diagnostic-only value.
...
- the first genuine corrected 7d diagnostic later showed `universe_requested=30`, `universe_resolved=16`, `supported_liquidity_subsets=[15]`, `minimum_subset_coverage=0.8`, `failed_symbol_count=14`, `research_blocked=true`, and `untouched_oos_opened=false`;
- this confirms the 7d blocker exactly: the second predeclared liquidity subset is not yet defensibly supported, so untouched OOS must remain closed. It does not constitute a failed strategy result.

PR #141 deployment verification:
- commit `149c11bf8e28f9cdf438405549e8a43478094102` reached `live` on both production and the continuous coordinator through normal main auto-deploy;
- aggregate failure-type evidence must be taken only from completed post-#141 ACC-002 runs; do not infer or backfill causes from prior failed-history counts.
...
Persistent specialist task priorities live in `orchestration/specialist_coordination.json`; specialists must follow current-main queue ownership/dependencies and may not use coordination state to bypass evidence gates. PRs #120/#121 and #141 are integrated and must not be re-applied. Old PRs #105, #34, #33 and #13 remain stale/non-mergeable against current main and must not be merged as-is. PR #140 proposes faster autonomous-specialist workflow cadence but its exact-head Security and Reliability run #1032 failed, so it must not be merged unless rebuilt/retested from current main and justified within the same cost/safety ceiling.

## EXACT NEXT STEP
1. Inspect the first genuine completed post-#141 24h and 7d ACC-002 bounded summaries and use their aggregate `failure_type_counts` to classify the missing-history cause without symbol identities or raw samples. Preserve `minimum_subset_coverage=0.80`, the two-supported-subset requirement, Top-N ordering, and unopened untouched OOS.
2. If failures are predominantly `InsufficientHistory`, remain fail-closed and let defensible coverage accumulate; never replace missing Top-N members with lower-ranked assets, shorten the required history merely to pass, or fabricate/backfill history. If transient/request exceptions dominate, isolate and repair only a confirmed collection defect and add a regression test before retesting.
3. Once canonical ACC-002 data/evidence preconditions are genuinely satisfied, run the PR #131 beta-neutral residual-momentum challenger only as a predeclared research experiment. Preserve untouched OOS, non-overlap, cost stress, bootstrap robustness, point-in-time safety, multiple-testing accounting and genuine forward proof.
4. Allow PR #129 prospective timestamped microstructure measurements and PR #123 future-only consensus provenance to accumulate naturally. Do not reconstruct historical books, infer hidden liquidity, backfill consensus, or claim precision/profitability before enough independent forward evidence resolves.
5. Monitor authentic paper cycles for `cross_horizon_symbol_conflict` suppression without rewriting historical trades.
...
7. Enable GitHub main branch/ruleset protection for defense in depth when repository-admin access is available; until then, preserve no-auto-merge and exact-head CI/manual review controls.
