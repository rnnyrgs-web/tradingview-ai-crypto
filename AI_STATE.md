...
Current tested integration baseline after PR #196: `85c6eb91f4fcd29a63988a130f2fbc15eead59a9`. This includes PR #194 research-only A+ meta-signal trust, PR #195 timestamp-safe consensus exposure, and PR #196 shadow-direction research eligibility on top of the prior economic-calibration/meta-WAIT and evidence-value scheduling work.
...
- A+ meta-signal trust uses immutable shadow LONG/SHORT direction as research eligibility and may evaluate rows whose production action remains WAIT; this is research-only and cannot mutate `action_at_forecast`, authorize a trade, open sealed OOS, or bypass the canonical chain;
...
Immediately before PR #196, both production and continuous coordinator were live on PR #195 main commit `a265182bfae4853325ea191c282aa5c1097762dd`. Coordinator evidence showed a healthy deployment canary, zero worker failures/timeouts, no stale/crashed workers, zero task restarts, 256 logical specialists and zero normal-operation specialist AI calls/errors. Trade/promotion/signal authority remained false. Deployment is operational evidence only; it is not a signal-quality or profitability claim.
...
## EXACT NEXT STEP
1. Verify PR #196 deployment reaches live on both Render services and the coordinator canary remains healthy with zero production/trade/promotion authority.
2. Inspect the first naturally completed post-#196 learning-worker A+ meta-signal-trust report. Confirm nonzero independent 24h shadow-direction development/validation samples while production-WAIT rows remain explicitly research-only and unmodified.
3. Never treat the 1,920 raw overlapping resolved 24h rows as independent evidence. Confirm meta-signal trust continues to use deterministic full-horizon non-overlap and keeps the final 20% untouched OOS sealed.
4. Measure timestamp-safe consensus coverage within independent 24h shadow-direction evidence. If consensus coverage is insufficient, wait for natural future-only accumulation; never reconstruct or backfill historical consensus.
5. Do not open A+ untouched OOS merely because development/validation looks promising. Any candidate must still pass predeclared validation, sequential multiple-testing, robustness/stability and the rest of the canonical chain.
6. Continue natural 7d outcome accumulation and preserve the ACC-002 natural-history blocker while aggregate failures remain `InsufficientHistory`; never weaken the 80% coverage, two-supported-subset, Top-N ordering or untouched-OOS rules.
7. Investigate the low shared-history cache hit rate only as a throughput question: determine whether misses reflect legitimate request diversity or exact repeated immutable history requests that can safely reuse provenance-identical completed-candle snapshots. Make no TTL/freshness change without exact-identity and chronology proof.
