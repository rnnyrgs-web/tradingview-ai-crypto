...
Current tested functional baseline after PR #251: `aafb06c0f78cc1d2c5ef5100988f3881f3720162`. PR #250 exact head `2c924b2f41ac52e02b04225abe14ef45e9dc4c7f` passed Security and Reliability #1777 before expected-head squash merge as `218a0aeda581504160eed720191d7841ae212745`. It preserves the existing one-per-base derivatives fetch while classifying Binance OI acquisition outcomes into a fixed non-sensitive aggregate vocabulary and propagating only those counts through the bounded adaptive-research boundary. PR #251 then projects those already-bounded counts through the coordinator's final log allowlist. Its first exact head correctly failed Security and Reliability #1782 because the strict complete-shape observability regression contract had not yet been updated; replacement exact head `0e1ac116b3670559b0c9f8992a47f849165a64b5` passed Security and Reliability #1784 before expected-head squash merge as `aafb06c0f78cc1d2c5ef5100988f3881f3720162`. Both changes add zero market-data requests and no signal, persistence, paper, promotion, broker, worker-count, paid-service, or recurring-cost authority.
...
PR #250 added bounded acquisition provenance using the already-returned `get_derivatives_history()` error metadata and the exact existing fetch. PR #251 made those counts visible at the final coordinator log boundary. Both Render services reached live on #250 and then on #251. The first post-#251 coordinator startup log correctly showed `oi_source_diagnostics=None` before the adaptive FFriZz worker had produced a new natural cycle; do not interpret startup absence as a source diagnosis. The next natural adaptive cycle is required before identifying the actual source failure class.
...
## EXACT NEXT STEP
1. Inspect the first natural post-#251 adaptive FFriZz cycle's bounded `oi_source_diagnostics`. Determine whether Binance OI acquisition is `available`, `valid_empty`, `http_error`, `timeout`, `network_error`, `invalid_payload`, or generic `source_error`; do not infer alpha from source availability.
2. If the source is failing, diagnose that specific acquisition class using the existing public feed and current request path before changing V3. Do not add retries, alternate feeds, paid services, relaxed timestamp rules, or extra recurring requests without evidence that they are necessary and safe.
3. If OI becomes available, evaluate V3 chronology/provenance first: preserve `<1H` staleness, no-future selection, no nearest-neighbour/interpolation, duplicate OI rejection, and ambiguous price-endpoint reuse rejection. Availability alone is not predictive usefulness and must not trigger threshold tuning.
...
12. Keep `live_promotions.json` empty, broker disconnected, signing keys unused, authentic paper baseline/history immutable, heavy concurrency bounded, and recurring infrastructure <= USD 30/month.
13. Every integration remains isolated branch -> regression tests -> exact-head Security and Reliability -> current-main compatibility -> expected-head merge -> AI_STATE.md synchronization -> post-deploy health verification when runtime code changes.
