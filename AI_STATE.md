...
The project uses the merged machine-readable routing policy at `orchestration/model_routing_policy.json`.
...
- OpenAI API `gpt-5.6-sol` is reserved for deep scientific/Lead review where the expected information value justifies the higher cost.
- GPT-6 Astra is routed through ChatGPT Work/Codex rather than the OpenAI API; the canonical Work instructions are in `docs/ASTRA_BACKGROUND_WORKER.md`.
- Background workers use GitHub as the mailbox/source of truth, not browser-tab scraping.
- If higher-capability interactive capacity is unavailable, checkpoint the exact handoff and continue independent deterministic/API/Claude work rather than stalling.
- The OpenAI cloud specialist may own both `data-market` and `quant-research` READY tasks but remains one model run at a time, one task per invocation, branch-isolated, budget-gated, unable to merge its own PR, and unable to trade. It follows the current canonical discovery task IDs/fingerprint rather than a hard-coded task generation.
...
The current reconciliation branch carries forward PR #416's sealed liquidity-screen evidence while preserving the merged background-orchestration work and repairing the state-transition CI contract.
...
1. Verify the reconciliation PR final head is based on current `main` and preserves the merged background-orchestration changes.
2. Require exact-head Security & Reliability success on the final PR head.
3. Require Strategy Discovery Supervisor validation to accept the durable liquidity rejection plus new `DISC-SQUEEZE-RETENTION-001-v1` ranked screen.
4. If those gates pass, merge only that exact verified head and verify the resulting canonical `main` SHA.
