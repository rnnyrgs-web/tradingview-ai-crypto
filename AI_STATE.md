...
## EXACT NEXT STEP
1. Verify an authenticated production workflow scan completes with two-source consensus where available, returns order-book evidence as research-only, fails closed where not, passes `tools/validate_scan_response.py`, and contains no unvalidated trade.
