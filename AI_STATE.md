...
This document intentionally consolidates superseded historical status into the current actionable state. Older PR-by-PR chronology remains available in Git history and merged PRs; durable negative research results and safety invariants are preserved below so they are not accidentally retried or weakened.
...
Headline accuracy, trade count, model count, worker count, or paper P&L are never optimization objectives by themselves. A lower-hit-rate strategy is acceptable only when rigorous chronological evidence shows superior after-cost expectancy with controlled risk. An accuracy gain must be rejected if it reduces net profitability.
...
PR #321 exact head `fd045ed3b4eefd3110737176b86f1909951c77b5` passed Security and Reliability #2205 before expected-head merge. The first PR head failed unit tests and was not merged; stale worker-binding and compatibility-test expectations were corrected without weakening safety behavior.
...
GitHub `main` branch protection remains disabled. Therefore isolated branches, exact-head Security and Reliability, current-main compatibility review, expected-head merge protection, and post-deploy verification are mandatory operational controls.
...
`live_promotions.json` remains empty. Broker remains disconnected. Signing keys remain unused. Do not add credentials or real-order capability without explicit user approval plus every canonical gate.
...
- Binance historical-OI acquisition from the current Render environment is confirmed blocked by HTTP 451. A per-run circuit breaker may stop redundant requests after one confirmed 451, but must not bypass the restriction.
- Bybit historical-OI accessibility from the current Render environment was confirmed HTTP 403 under the bounded probe. Do not repeatedly re-probe the unchanged route unless deployment/network environment, access policy, or endpoint contract materially changes.
...
- FFriZz V3 causal-as-of alignment remains a separate hypothesis, not alpha evidence; it remained unavailable while OI itself was unavailable. Preserve `<1H` staleness, no-future selection, no nearest-neighbour/interpolation, duplicate-OI rejection, and ambiguous endpoint-reuse rejection.
...
PR #319 made scarce research allocation genuinely profitability-first. PR #320 made the central research director explicitly prioritize expected after-cost profitability impact, with signal quality/accuracy only secondary. No profitability improvement was claimed from those control-plane changes.
...
After #321 the active logical worker roster is 20 workers: 18 heavy and 2 lightweight. Physical heavy concurrency and the existing cost ceiling were not increased. The roadmap top-level objective is now explicitly profitability-first, and active worker bindings no longer include the retired rejected-candidate process.

`COORD-DATA-005` is the next READY data-market task: select at most one genuinely new timestamp-safe profitability candidate that is materially distinct from rejected DATA-BASIS-001 and DATA-FUNDING-001, states its economic mechanism before testing, uses a bounded free/public path or existing timestamp-safe data, and is evaluated separately at 24h and 7d with chronological non-overlapping evidence and realistic costs. Reject rather than tune if incremental after-cost evidence fails.

Do not retry unchanged Binance or Bybit historical-OI routes as the next candidate.

## ACC-002 / CROSS-ASSET STATUS
ACC-002 remains governed by the existing fail-closed liquidity/history rules. Never lower `minimum_subset_coverage=0.80`, required supported-subset count, Top-N ordering, required history, chronology, or untouched-OOS rules merely to pass.
...
- 7d remained blocked for `insufficient_supported_liquidity_subsets`, with roughly 19/30 histories resolving and the remaining failures classified as `InsufficientHistory`; untouched OOS remained unopened.
...
Recent research found an economically interesting but still under-sampled pattern in 7d execution: losing stop-outs showed materially worse measured entry slippage than measured target winners. Direction-aware microstructure research and incremental normal-execution baselines were added in later PRs so this can be tested prospectively rather than converted into an overfit cutoff. No production slippage threshold is authorized from the small sample.
...
At this state sync there are no known open PRs after #321 integration.

Highest-value next actions:
1. Start from current `main` and re-read this file plus `orchestration/specialist_coordination.json`.
2. Prefer `COORD-DATA-005` or another clearly higher expected-profitability-impact falsifiable experiment only if it is genuinely new and timestamp-safe.
3. Keep 24h and 7d economics separate. Focus scarce compute on data-ready 24h mechanisms while 7d remains natural-history blocked, unless material new evidence changes that ranking.
4. Continue diagnosing losing trades and missed profitable opportunities into predeclared hypotheses: selective WAIT, economic calibration, regime conditioning, cross-sectional residual signals, execution/adverse-selection vetoes, feature removal, and strategy selection.
5. Require incremental after-cost value versus an appropriate normal/frozen baseline; do not blame execution conditions for losses when the underlying strategy/regime itself is weak.
6. Preserve direction-aware microstructure logic and separate horizon baselines.
7. Reject accuracy gains that reduce net expectancy, profit factor, or risk-adjusted return.
8. Preserve negative results and do not repeat disproven candidates absent a material condition change.
9. Keep `live_promotions.json` empty, broker disconnected, authentic paper ledger append-only, and recurring infrastructure <= USD 30/month.
10. Every integration remains isolated branch -> regression tests for confirmed defects -> exact-head Security and Reliability green -> current-main compatibility -> expected-head merge -> AI_STATE synchronization -> post-deploy health verification when runtime code changes.

## LATEST INTEGRATION — PR #321
PR #321 is a research-efficiency/governance integration, not an alpha result.

It removed a retired compatibility process for already-rejected DATA-BASIS/DATA-FUNDING research from the always-on heavy/accuracy lane, removed the obsolete active binding/recheck wiring, aligned the roadmap with profitability-first priority, and added regression coverage so the rejected candidate cannot silently re-enter the active worker army.
