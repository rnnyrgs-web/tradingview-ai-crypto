...
Last reconciled: 2026-09-20T09:04Z
Last updated: 2026-09-20T09:04Z
...
**PR #482 = INTEGRATED BOUNDED PHASE-3 REPAIR WITH OPEN SCIENTIFIC AND RUNTIME BLOCKERS.** After an adversarial regression reproduced a fresh significance allowance for each relabeled causal family, the Lead merged exact PR head `0bbc7b8d37ee4e6496dd5065be1a2375b6ec1c08` from `main` `2bfd2923b80985310a6bc48bad1856810552f74f` by GitHub merge commit to `572bb6ac2d4d802e844d3fdc459b86df9e2a8acc`. GitHub records PR #482 merged and main at that SHA; its two merge parents and all scientific source/test/workflow/migration bytes match the reviewed head. The earlier independent clearance was superseded before merge by a reproduced Important finding: the frozen evaluation units omitted exact matched-control identity, so six different post-outcome controls sharing the same unit coordinates could be substituted, accepted as confirmatory support and persisted across restart. Exact-head Security & Reliability run `35500161031` and exact merged-main run `35500312414` both succeeded in the unit, dependency, static-security and secret-scan lanes, but those tests did not close the control-substitution attack. Profitability Learning Runtime Acceptance run `35500312440` succeeded on that exact main. Money Intelligence Causal Runtime Acceptance run `35500312412` **FAILED** on HTTP 500 despite an exact-SHA live Render deployment. The deployed stack trace shows strict family-size validation cannot restore four legacy acceptance hypotheses in one family, each frozen with `family_size=2`. This is a fail-closed scientific-contract stop, not runtime acceptance; it requires a compatible historical-memory repair and fresh exact-deployed verification. The existing 6-pair acceptance fixture also cannot pass the new project slot threshold and must be redesigned prospectively without opening market OOS/forward evidence.

**PR #486 = OPEN CORRECTIVE MATCHED-CONTROL PROVENANCE REPAIR.** It freezes exact ordered `(outcome_observation_id, control_observation_id)` pairs before outcomes, includes them in design identity and research artifacts, requires confirmatory evidence to match them exactly, preserves rejected-design memory across the schema strengthening, and removes confirmatory authority from restored historical support that lacks pair provenance. Direct and durable Supabase/restart regressions reject the reproduced substitution attack. Exact implementation head `b2cf92abdd9657bd73213aacdce5079477e8076b` passed Security & Reliability run `35500973351`; focused verification passed 79 tests and the full local suite passed 1,351 tests / 2 skipped. PR #486 remains unmerged and requires synchronization with current `main`, fresh exact-head CI and independent review before Lead-only integration.
...
Continue the already-claimed `COORD-ARCH-ADVERSARIAL-001` from exact current `main` under issue #462. First synchronize corrective PR #486, require fresh exact-head Security & Reliability and independent review, and Lead-integrate it only if the matched-control substitution attack remains closed. Then repair the separate legacy-family restore defect and acceptance-fixture significance mismatch, rerun exact-deployed causal acceptance, and preserve WAIT until genuinely prospectively frozen synthetic units mature. Next close PR #480's known-fingerprint strategy/science-design omission bypass before any semantic-identity integration. Continue the remaining adversarial falsification afterward. Do not self-certify Phase 3, advance V2-002/Phase 4 ahead of the active milestone, reopen protected OOS/forward evidence, or grant broker/trade/promotion authority.
