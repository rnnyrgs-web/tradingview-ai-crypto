...
- PR #66 `ACC-002: Reserve an accuracy research fast lane` adds a fixed one-slot ACC-002 lane plus one general-research slot inside the existing hard concurrency limit of two. This prevents majors/PONS/universe jobs from delaying both 24h/7d evidence loops for many hours, without adding infrastructure or weakening any evidence gate. Exact-head CI and merge/deploy verification are pending.
...
## EXACT NEXT STEP
1. Merge/deploy PR #66 only after its updated exact head passes Security and Reliability. Verify both Render services reach `live` and `/workers` reports `accuracy_reserved_slots=1`, total heavy concurrency no higher than two and all authorities false.
