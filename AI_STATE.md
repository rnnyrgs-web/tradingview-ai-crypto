...
Current tested functional baseline after PR #253: `ac6d1cbcc6c4ab3ac1879dfd0a86c9b8d9e58c40`. PR #253 exact head `41c497a2f5f9d98073212ab65196d09ee7194089` passed Security and Reliability #1794 before expected-head merge as `ac6d1cbcc6c4ab3ac1879dfd0a86c9b8d9e58c40`. It adds an allowlisted, non-sensitive HTTP status bucket only to the already-existing Binance OI-history acquisition failure path so the repeated `http_error` condition can be distinguished prospectively as `http_451`, `http_429`, `http_other_4xx`, `http_5xx`, or `http_other`. It adds zero requests, zero retries, no alternate provider, no raw URL/payload/error text, and no signal, persistence, paper, promotion, broker, worker-count, paid-service, or recurring-cost authority. Regression tests verify fixed bucketing, single-request behavior, and that symbol/URL detail does not escape. Both Render services reached live on the functional merge.
...
PR #253 safely narrows that uncertainty without changing acquisition behavior. It logs only one fixed allowlisted HTTP status bucket from the already-raised `HTTPStatusError`; both Render services are live on the functional merge. At state-synchronization time no natural post-#253 FFriZz acquisition had yet emitted the new bucket, so no exact HTTP cause is claimed. Do not infer 451 merely from deployment geography or external plausibility.
...
## EXACT NEXT STEP
1. Observe the first natural post-#253 Binance OI-history failure bucket from the live coordinator. Treat only the emitted allowlisted class (`http_451`, `http_429`, `http_other_4xx`, `http_5xx`, or `http_other`) as evidence; do not infer the cause from deployment geography or external anecdotes.
