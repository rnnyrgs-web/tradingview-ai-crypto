...
The candidate includes deterministic tests for fresh consensus, exchange disagreement, staleness, insufficient sources, multi-source funding and liquidation parsing. Publication, exact-SHA CI/security review and production deployment verification remain pending.
...
## EXACT NEXT STEP
1. Publish the cross-exchange/derivatives candidate through a PR, require full unit/security checks on the exact candidate SHA, merge only if green, then verify the Render deploy and a production scan remain healthy and contain no unvalidated trade.
2. After the fixed deadlines pass, verify the first 24h and 7d ledger outcomes use only candles at or after `due_at`; confirm calibration remains restrictive until each comparable bin has at least 30 resolved forecasts and its 95% Wilson lower bound is at least 50%.
3. Verify the hardened release deploys successfully and the next production scan passes `tools/validate_scan_response.py`, produces no unvalidated `TRADE` / BUY / SELL decision, and exposes a healthy sanitized `/health` operational snapshot.
