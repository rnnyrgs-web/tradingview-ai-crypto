...
Parallel ChatGPT development coordination exists through `AGENTS.md`, `docs/CHATGPT_SPECIALISTS.md`, and the persistent fail-closed `orchestration/specialist_coordination.json` queue integrated by PR #120. The autonomous lead remains review/integration-controlled: no specialist may auto-merge, write directly to main, connect a broker, increase recurring cost, or weaken canonical evidence gates.
...
PR #136 is an observability-only correction to PR #135. Regression coverage models the actual bounded producer summary shape, verifies top-level supported-subset and failed-count handling, retains sanitized failure-type aggregation compatibility, and confirms the threshold is the unchanged canonical 80% constant rather than a loosened diagnostic-only value.

PR #120 was concurrently refreshed onto the then-current post-#136 main, exact head `d56620ed0a74f650f87c679fadc8e9726f851f33` passed Security and Reliability run `34356132342` (#1007), and it was merged as `b9765adabfa1dde0dae3aa747a406421292a3631`. It adds a persistent fail-closed specialist coordination queue and validation helper without auto-merge, broker connectivity, additional infrastructure/heavy concurrency, production threshold changes, or weakened validation gates. The coordination queue prioritizes research/development work; it does not itself authorize evidence, promotion, or production behavior.
...
- genuine post-deployment runs showed 24h repeatedly resolving about 24-26 of 30 requested assets and 7d about 14-16 of 30, while both remained fail-closed with `insufficient_supported_liquidity_subsets` and untouched OOS unopened;
...
- PR #121 — autonomous cloud specialist runner v1 remains stacked on the former PR #120 branch and must not be merged as-is. Re-sync/retarget it onto current main and rerun fresh exact-head Security and Reliability before any integration. Preserve review-only/no-auto-merge behavior and the USD 30/month ceiling.
- Persistent specialist task priorities now live in `orchestration/specialist_coordination.json`; specialists must follow current-main queue ownership/dependencies and may not use coordination state to bypass evidence gates.
- stale PRs #105, #113, #114, #115, #117, #118 and #119 are superseded/closed or otherwise stale and must not be merged as-is.

## EXACT NEXT STEP
1. Verify the PR #136 coordinator deployment and inspect the first genuine post-#136 24h and 7d ACC-002 summaries. Use only bounded `universe_requested`, `universe_resolved`, `supported_liquidity_subsets`, unchanged `minimum_subset_coverage=0.80`, `failed_symbol_count`, and aggregate failure-type evidence if actually available. Determine whether the current `insufficient_supported_liquidity_subsets` block is genuine insufficient top-N/history coverage or a data-collection defect. Do not lower the 80% coverage or two-subset requirement merely to open OOS.
...
8. Review PR #121 separately for coordination value versus added complexity/cost; preserve no-auto-merge and the $30/month ceiling.
