...
Current tested functional integration baseline after PR #229: `ff355d411698aafb9a086026d43dfb72fdadcd7c`.
...
A read-only Supabase audit during the #229 cycle found zero FFriZz rows in `prediction_ledger`. This is consistent with the subsequent live coordinator evidence because, after #227, repeated sampled FFriZz cycles between roughly 2026-09-10 11:40Z and 12:35Z reported `collection_ok=true`, `error_type=None`, and `eligible_shadow_forecasts=0`. These are legitimate source-abstention cycles, not persistence failures. They are not signal-performance evidence and are not a reason to lower thresholds.

The same sampled coordinator window remained operationally healthy with zero worker failures and zero worker timeouts. ACC-002 remained naturally blocked by `InsufficientHistory`: the sampled 24h run resolved 27/30 with three pure insufficient-history failures and only the Top-15 subset supported; the sampled 7d run resolved 13/30 with 17 pure insufficient-history failures and no supported subset. Untouched OOS remained closed. Do not convert this natural-history limitation into a software failure unless failure types materially change.
...
## EXACT NEXT STEP
1. After #229 is live on the research coordinator, inspect the bounded FFriZz abstention diagnostics from a natural cycle. Confirm action counts reconcile with `eligible_shadow_forecasts`, fixed-gate WAIT counts sum correctly, unavailable-family reasons are factual, and `unexpected_wait_state` remains zero.
2. Do not lower FFriZz thresholds merely because source activity is sparse. Use the diagnostic to decide whether scarcity is due to insufficient directional agreement, below-threshold magnitude, missing OI/other feature inputs, or actual rule drift.
3. If a natural cycle later has eligible forecasts >0, verify canonical prediction-ledger rows read-only: deterministic UUID scan identity, LONG/SHORT direction, canonical WAIT action, score within 0..100, signed `calibration.raw_score`, preserved `shadow_action`, exact due_at, and full-horizon bucket identity. Any missing persistence fails closed.
4. Let valid prospective rows resolve naturally before any precision/expectancy comparison. Use only exact fingerprint, non-overlapping full-horizon evidence and realistic costs. No unresolved-row tuning or production promotion.
5. Continue using resolved primary-system errors, calibration, regime behavior, cross-sectional information, microstructure, execution realism and research-memory outcomes to prioritize falsifiable high-information experiments. Prefer evidence generation over cosmetic changes.
6. Treat history-network/cache optimization as a throughput experiment only. Never stretch TTL/freshness, change completed-candle semantics, collapse distinct request identities, or weaken source/provenance checks to increase a cache hit rate.
