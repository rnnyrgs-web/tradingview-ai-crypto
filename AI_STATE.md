...
Current tested functional integration baseline after PR #239: `46dd83e6e9e8b2ab0a69a43c7de1fdf5f3f8e0a2`. PR #239 exact head `8305cb81e083709d37567783c7470f1c840a7592` passed Security and Reliability #1716 before squash merge. It fixes calibration evidence freshness by resolving due forecasts before the next production `/scan`, so the newly built opportunity snapshot can use all just-resolved ledger evidence. The existing 30-sample calibration gate, non-overlapping full-horizon independence rules, signal thresholds and trading authority remain unchanged.
...
## SAFETY INVARIANTS
No AI opinion, ranking/evidence score, dashboard value, paper P&L, model count, ensemble weight, order-book snapshot, historical diagnostic, research-memory lesson, experiment priority, FFriZz signal/abstention count, prospective row, V2 feature-availability count, or single OOS/forward result may authorize live BUY/SELL by itself.
...
- FFriZz V2 OI-close-end challenger remains diagnostic-only until its prospective feature availability and chronology are observed cleanly; no V2 forward rows or promotion evidence exist from #236.
...
PR #235 predeclared `FFRIZZ_SECONDARY_V2_OI_CLOSE_END` as a separate challenger whose only intended feature change is to align each completed 1H candle CLOSE endpoint exactly to the Binance OI period END endpoint. It rejects unsupported bars, ambiguous duplicate endpoints and insufficient exact overlap, and uses no interpolation/backfill. Post-merge inspection found #235 did not actually invoke that challenger from the live FFriZz runtime despite its orchestration descriptor. PR #236 corrected that integration defect and regression-tested the runtime invocation. V2 now runs only for 6h/12h/24h using the same cached 1H candles/OI already used by the V1 pass, and its result is reduced to bounded count-only feature-availability evidence. The first natural live post-#236 V2 diagnostic has still not been established from the sampled standard `ffrizz_forward` observability output; those sampled logs expose V1 family-count distributions but no separately identifiable V2 field. Do not infer V2 feature availability from the V1 `available_family_count_distribution`; inspect the V2-specific observability path before drawing a conclusion.
...
Post-#237 coordinator sampling through approximately `2026-09-10T16:08Z` remained operationally healthy: worker failures=0, worker timeouts=0, supervisor healthy, no stale/crashed workers and no task restarts. ACC-002 remained fail-closed under pure `InsufficientHistory`; untouched OOS stayed closed. FFriZz sampled cycles were all-WAIT with `unexpected_wait_state=0`. These samples do not clear the known eligible-write `PredictionLedgerNotConfigured` blocker because no persistence write was required.

PR #239 corrected a separate calibration-freshness defect in `.github/workflows/crypto_scan.yml`: due forecasts are now evaluated before the new production scan. Before #239, the new opportunity snapshot could be calculated from ledger state that was one scan cycle stale because `/evaluate` ran afterward. This was an evidence-timing defect, not a signal-threshold defect. The exact-head tests enforce evaluate-before-scan ordering while preserving all calibration sample-sufficiency and independence rules. Both Render services began automatic deployment of merge `46dd83e6e9e8b2ab0a69a43c7de1fdf5f3f8e0a2`; deployment health must be confirmed before treating #239 as live operational behavior.
...
#235/#236/#239 and older FFriZz PRs #214/#216/#217/#219/#221/#223/#225/#227/#229/#231 are integrated and must not be re-applied. #230/#232/#233/#234/#237 are state-only/reconciliation work. PR #238 is exact-head green on its own head but remains unmerged pending current-main compatibility review after #239. Stale PRs #34, #33 and #13 remain incompatible with current main and must not be merged as-is without fresh relevance/compatibility review.
...
## EXACT NEXT STEP
1. Verify both Render services are live on PR #239 merge `46dd83e6e9e8b2ab0a69a43c7de1fdf5f3f8e0a2`, then verify the next scheduled production workflow actually resolves due forecasts before the scan and that fresh calibration evidence is reflected without weakening the 30-sample/non-overlap gates.
...
3. Preserve the current FFriZz V1 fingerprint and thresholds. Do not reinterpret the 57/15 abstention split or the later three SELLs as evidence for threshold tuning.
4. Locate and inspect the post-#236 V2-specific count-only observability path for 6h/12h/24h. Verify `price_oi_correlation_v2` availability by horizon, that diagnostics remain bounded/symbol-free, and that V2 uses no interpolation/backfill or authority. Do not substitute V1 family-count diagnostics for V2 evidence. If OI remains unavailable, diagnose exact endpoint/cadence/data-source provenance rather than loosening matching.
...
8. Continue using resolved primary-system errors, calibration, regime behavior, cross-sectional information, microstructure, execution realism and research-memory outcomes to prioritize falsifiable high-information experiments.
