...
Current tested functional baseline after PR #257: `5e154a6b0fb7b736528a78329691ecadcb1566d0`. PR #257 exact head `3565444f8ae07774b076b17f31110c5a72c50dc1` passed Security and Reliability #1813 before expected-head squash merge. It fixes the confirmed OI HTTP-status propagation defect left after #253: the low-level Binance OI acquisition already emitted one bounded `http_status_bucket`, but `ffrizz_secondary_runner.py` discarded it and reclassified only the exception name as generic `http_error`, while the adaptive coordinator boundary did not allow the new status buckets. FFriZz now preserves only the predeclared `http_451`, `http_429`, `http_other_4xx`, `http_5xx`, and `http_other` buckets with generic fallback for absent/unrecognized values, and the adaptive boundary re-allowlists the same aggregate vocabulary. Regression tests cover propagation, generic fallback, and exclusion of arbitrary status/symbol/URL details. No extra requests, retries, provider changes, signal/persistence/paper/promotion/broker authority, worker-count, paid-service, or recurring-cost change was introduced.
...
PR #253 safely narrowed that uncertainty at the acquisition layer, but natural post-#253 cycles still surfaced only generic aggregate `http_error`. Inspection during the #257 cycle identified the exact cause: `market_data.py` already carried `http_status_bucket`, while FFriZz ignored it and the adaptive coordinator sanitizer did not allow the new bucket vocabulary. PR #257 fixes those two propagation boundaries without changing acquisition behavior. At state-synchronization time no natural post-#257 live FFriZz request has yet emitted the final bucket, so no exact Binance HTTP cause is claimed.
...
## EXACT NEXT STEP
1. Observe the first natural post-#257 Binance OI-history status bucket at the final live coordinator boundary. Treat only the emitted allowlisted class (`http_451`, `http_429`, `http_other_4xx`, `http_5xx`, or `http_other`) as evidence; do not infer the cause from deployment geography or external anecdotes.
2. If the bucket is `http_451`, keep the source fail-closed and evaluate compliant zero/low-cost supported data/deployment alternatives before any infrastructure change; never bypass geographic restrictions. If `http_429`, diagnose request-rate pressure and reuse opportunities before adding retries or requests. If `http_5xx`, treat it first as an upstream/transient availability condition. If `http_other_4xx`, inspect documented request semantics before changing code.
...
8. Continue using resolved primary-system errors, calibration, regime behavior, cross-sectional information, market microstructure, execution realism, paper rejection attribution, and research-memory outcomes to rank falsifiable experiments by expected information gain and after-cost impact. Recent 2026 evidence on crypto order flow and clock-phase/quarter-hour effects is hypothesis-generating only; any local experiment must use defensible timestamped data, purged chronological validation, realistic execution costs, and multiple-testing protection before it can affect strategy selection.
