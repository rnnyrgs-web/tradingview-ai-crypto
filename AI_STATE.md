...
Current tested functional research baseline after PR #244: `99b5e1eb0639022a8d14ef728341517ba8d9cde4`. PR #244 exact head `67646605e20af3c363ea69a39bc7e958b5f5b62c` passed Security and Reliability #1743 before expected-head squash merge after current-main compatibility was rechecked. It records the failed FFriZz V2 exact OI/candle-end alignment hypothesis and adds the separately fingerprinted research-only `FFRIZZ_SECONDARY_V3_OI_CAUSAL_ASOF` challenger. V3 uses only the latest completed 1H price endpoint at or before each OI timestamp, requires staleness strictly below one bar, rejects ambiguous reused price endpoints, and forbids future-price, nearest-neighbour, or interpolation semantics. It is not runtime-wired yet and has no persistence, production, paper, promotion, or broker authority.
...
- FFriZz V3 causal-as-of challenger is predeclared and unit-tested but not runtime-wired as of #244. It must first demonstrate prospective availability/chronology under bounded diagnostics before any forward-ledger design or signal-quality comparison is considered.
...
## EXACT NEXT STEP
1. Runtime-wire V3 only as a bounded research diagnostic for 6h/12h/24h, reusing the existing 1H candle/OI cache. Expose only count-level V3 feature availability plus bounded causal-staleness/integrity diagnostics; do not persist V3 forecasts or create paper/production/promotion/broker authority.
2. Before interpreting V3 availability, regression-test that no future price endpoint can be selected, staleness must remain `<1H`, and reused price endpoints fail closed. Keep V1/V2/V3 evidence separate.
3. If V3 restores OI-family availability prospectively, first verify timestamp/staleness distributions and data provenance. Availability alone is not predictive usefulness and must not trigger threshold tuning.
