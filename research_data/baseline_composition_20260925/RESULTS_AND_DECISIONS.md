...
- A loader restricted to canonical `evidence.json.gz` before any read, with exact compressed-byte hash validation before decompression and original payload hash validation afterward. It never opens `dataset.json.gz`, recomputes signals, searches thresholds, reads sensitivities or accesses fresh/OOS/forward outcomes. Independent review caught the initial check-after-read flaw; three synthetic boundary attacks failed before repair and pass afterward.
