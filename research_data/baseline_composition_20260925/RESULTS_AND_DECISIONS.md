...
- A loader pinned to the original consumed payload hash. It reads only `evidence.json.gz`; it never opens `dataset.json.gz`, recomputes signals, searches thresholds, reads sensitivities or accesses fresh/OOS/forward outcomes.
