...
def _worker_evidence(row: dict[str, Any]) -> dict[str, Any]:
    evidence = row.get("latest_evidence"); return evidence if isinstance(evidence, dict) else {}
def _pure_history_block(evidence: dict[str, Any]) -> bool:
    failure_types = evidence.get("failure_type_counts"); return evidence.get("research_blocked") is True and isinstance(failure_types, dict) and bool(failure_types) and set(failure_types) == {"InsufficientHistory"}
def _evidence_probability(evidence: dict[str, Any], fallback: float) -> float:
    if not evidence: return fallback
    conclusion = str(evidence.get("evidence_conclusion") or "")
    if conclusion in {"validation_failed", "oos_evaluated", "validation_passed"}: return 0.95
...
def _daily_report(army: dict[str, Any], missions) -> dict[str, Any]:
    workers = army.get("workers") if isinstance(army.get("workers"), dict) else {}; supervisor = army.get("supervisor") if isinstance(army.get("supervisor"), dict) else {}; adaptive = _worker_evidence(workers.get("adaptive-accuracy") or {}); experiment = adaptive.get("experiment") if isinstance(adaptive.get("experiment"), dict) else {}; conclusion = str(adaptive.get("evidence_conclusion") or ""); observability = army.get("observability") if isinstance(army.get("observability"), dict) else {}; worker_obs = observability.get("workers") if isinstance(observability.get("workers"), dict) else {}; wait_findings=[]
    if experiment and str(experiment.get("dimension")) in {"score_band", "market_regime", "direction", "strategy_identity"}: wait_findings.append(f"adaptive restrictive test dimension={experiment.get('dimension')} group={experiment.get('group')} conclusion={conclusion or 'pending'}")
    return build_daily_lead_report(jobs_considered=len(missions), jobs_started=sum(1 for row in workers.values() if isinstance(row, dict) and row.get("state") in {"queued", "running"}), jobs_completed=int(army.get("completed_jobs") or 0), worker_failures=int(worker_obs.get("failed") or army.get("failed_jobs") or 0), timeouts=int(worker_obs.get("timeouts") or 0), restarts=int(supervisor.get("task_restarts") or 0), experiments_tested=1 if experiment else 0, experiments_rejected=1 if conclusion == "validation_failed" else 0, experiments_blocked=sum(1 for mission in missions if mission.blocker), validation_passed=1 if experiment.get("validation_passed") is True else 0, untouched_oos_opened=1 if adaptive.get("oos_opened") is True else 0, metrics={"worker_count": int(army.get("worker_count") or 0), "heavy_worker_count": int(army.get("heavy_worker_count") or 0), "lightweight_worker_count": int(army.get("lightweight_worker_count") or 0), "active_jobs": int(army.get("active_jobs") or 0)}, wait_findings=wait_findings, execution_findings=[], next_missions=rank_missions(missions)[:5], production_promotion=False)


def _write_state(payload: dict[str, Any]) -> None:
    try:
        _STATE_PATH.parent.mkdir(parents=True, exist_ok=True); fd, tmp = tempfile.mkstemp(prefix="research-director-", suffix=".tmp", dir=_STATE_PATH.parent)
        try:
            with os.fdopen(fd, "w", encoding="utf-8") as handle: json.dump(payload, handle, sort_keys=True, separators=(",", ":")); handle.flush(); os.fsync(handle.fileno())
...
def snapshot() -> dict[str, Any]:
    with _lock: return json.loads(json.dumps(_state))
