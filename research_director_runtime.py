...
def _pure_history_block(evidence: dict[str, Any]) -> bool:
    ...
    return evidence.get("research_blocked") is True and isinstance(failure_types, dict) and bool(failure_types) and set(failure_types) == {"InsufficientHistory"}
...
def _evidence_probability(evidence: dict[str, Any], fallback: float) -> float:
    ...
    if conclusion in {"validation_failed", "oos_evaluated", "validation_passed"}:
        return 0.95
    if conclusion in {"no_dispatchable_hypothesis", "insufficient_evidence", "insufficient_history"}:
        return 0.25
    if evidence.get("research_blocked") is True:
        return 0.15
    return fallback
...
def _mission_for_worker(name: str, row: dict[str, Any]):
    ...
    if name == "adaptive-accuracy":
        ...
        dispatchable = evidence.get("evidence_conclusion") not in {"no_dispatchable_hypothesis", None}
    ...
    if name == "learning-diagnostics":
        return build_mission(lane="learning-diagnostics", horizon="both", direction="BUY_SELL_WAIT", theme="resolved_error_diagnostics", hypothesis="diagnose fresh resolved-signal mistakes into falsifiable mechanisms", expected_information_gain=0.82, expected_signal_impact=0.78, sample_readiness=0.85, novelty=0.8, falsification_value=0.9, actionable_evidence_probability=_evidence_probability(evidence, 0.8), compute_cost=0.15)
    if name == "experiment-factory":
        return build_mission(lane="experiment-factory", horizon="both", direction="BUY_SELL_WAIT", theme="hypothesis_generation", hypothesis="convert fresh diagnostics into ranked predeclared experiments", expected_information_gain=0.84, expected_signal_impact=0.8, sample_readiness=0.8, novelty=0.9, falsification_value=0.92, actionable_evidence_probability=_evidence_probability(evidence, 0.8), compute_cost=0.15)
...
def _daily_report(army: dict[str, Any], missions) -> dict[str, Any]:
    ...
    if experiment and str(experiment.get("dimension")) in {"score_band", "market_regime", "direction", "strategy_identity"}:
        wait_findings.append(f"adaptive restrictive test dimension={experiment.get('dimension')} group={experiment.get('group')} conclusion={conclusion or 'pending'}")
...
def refresh_director(army: dict[str, Any]) -> dict[str, Any]:
    ...
    visible_missions = rank_missions(missions)
