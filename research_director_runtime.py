...
from bybit_oi_access_probe import ALLOWED_STATUSES as BYBIT_OI_ALLOWED_STATUSES
from bybit_oi_access_probe import SOURCE_ID as BYBIT_OI_SOURCE_ID
from bybit_oi_access_probe import SYSTEM_ID as BYBIT_OI_SYSTEM_ID
from bybit_oi_access_probe import probe as probe_bybit_oi_access
...
def _compact_bybit_probe(raw: object) -> dict[str, Any]:
    """Re-allowlist the one-shot accessibility result at the director boundary."""
    row = raw if isinstance(raw, dict) else {}
    status = row.get("status")
    if status not in BYBIT_OI_ALLOWED_STATUSES:
        status = "source_error"
    points = row.get("points_observed")
    if not isinstance(points, int) or isinstance(points, bool):
        points = 0
    points = max(0, min(points, 5))
    return {
        "system": BYBIT_OI_SYSTEM_ID,
        "diagnostic_only": True,
        "source": BYBIT_OI_SOURCE_ID,
        "status": status,
        "points_observed": points,
...
def _ensure_bybit_probe() -> dict[str, Any]:
    """Run the reviewed public accessibility probe no more than once per process."""
    global _bybit_probe_ran, _bybit_probe_result
    with _lock:
        if _bybit_probe_ran:
            return dict(_bybit_probe_result or _compact_bybit_probe({}))
        _bybit_probe_ran = True
    try:
        raw = probe_bybit_oi_access()
    except Exception:
        raw = {"status": "source_error", "points_observed": 0}
    result = _compact_bybit_probe(raw)
    with _lock:
        _bybit_probe_result = result
    return dict(result)
...
def _mission_for_worker(name: str, row: dict[str, Any]):
    ...
    if name == "adaptive-accuracy":
        ...
        return build_mission(
            ...
            sample_readiness=0.9 if dispatchable else 0.55,
            novelty=0.9,
            falsification_value=0.98,
            actionable_evidence_probability=_evidence_probability(evidence, 0.85 if dispatchable else 0.4),
            compute_cost=0.55,
            blocker=blocker,
    ...
    if name in {"cross-asset-rank-24h", "cross-asset-rank-7d"}:
        ...
        return build_mission(
            ...
            actionable_evidence_probability=_evidence_probability(evidence, readiness),
            compute_cost=0.8,
            blocker=blocker,
        )
    if name == "learning-diagnostics":
        return build_mission(
            ...
            novelty=0.8,
            falsification_value=0.9,
            actionable_evidence_probability=_evidence_probability(evidence, 0.8),
            compute_cost=0.15,
        )
    ...
    return build_mission(
        ...
        novelty=0.58,
        falsification_value=0.62,
        actionable_evidence_probability=_evidence_probability(evidence, 0.55),
        compute_cost=0.65,
        blocker=blocker,
    )
...
def _daily_report(army: dict[str, Any], missions) -> dict[str, Any]:
    ...
    return build_daily_lead_report(
        ...
        experiments_tested=1 if experiment else 0,
        experiments_rejected=1 if conclusion == "validation_failed" else 0,
        experiments_blocked=sum(1 for mission in missions if mission.blocker),
        validation_passed=1 if experiment.get("validation_passed") is True else 0,
        untouched_oos_opened=1 if adaptive.get("oos_opened") is True else 0,
        metrics={
...
def refresh_director(army: dict[str, Any]) -> dict[str, Any]:
    ...
    workers = army.get("workers") if isinstance(army.get("workers"), dict) else {}
    missions = [
        _mission_for_worker(name, row if isinstance(row, dict) else {})
    ...
    payload = {
        ...
        "bybit_oi_access_probe": bybit_probe,
        "research_only": True,
        "trade_authority": False,
        "promotion_authority": False,
