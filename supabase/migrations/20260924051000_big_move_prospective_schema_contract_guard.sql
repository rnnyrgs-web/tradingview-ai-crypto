...
begin
  ...
  -- Freeze the complete authoritative column surfaces (type/nullability/identity).
  select string_agg(
           a.attname || ':' || pg_catalog.format_type(a.atttypid, a.atttypmod) || ':' ||
           case when a.attnotnull then 'notnull' else 'nullable' end || ':' ||
           case when a.attidentity = '' then '-' else a.attidentity end,
           ',' order by a.attnum
         )
    into v_sig
    from pg_catalog.pg_attribute a
   where a.attrelid = v_ref and a.attnum > 0 and not a.attisdropped;
  if v_sig is distinct from
     'sequence:bigint:notnull:a,asset_id:text:notnull:-,source_id:text:notnull:-,reference_price:numeric:notnull:-,observed_at:timestamp with time zone:notnull:-,captured_at:timestamp with time zone:notnull:-,evidence_sha256:text:notnull:-,evidence:jsonb:notnull:-,created_at:timestamp with time zone:notnull:-' then
  ...
  select string_agg(
    ...
    into v_sig
    from pg_catalog.pg_attribute a
   where a.attrelid = v_form and a.attnum > 0 and not a.attisdropped;
  if v_sig is distinct from
     'sequence:bigint:notnull:a,forecast_fingerprint:text:notnull:-,reference_observation_sequence:bigint:notnull:-,formation_payload:jsonb:notnull:-,created_at:timestamp with time zone:notnull:-' then
  ...
  select string_agg(
    ...
    into v_sig
    from pg_catalog.pg_attribute a
   where a.attrelid = v_out and a.attnum > 0 and not a.attisdropped;
  if v_sig is distinct from
     'sequence:bigint:notnull:a,formation_sequence:bigint:notnull:-,forecast_fingerprint:text:notnull:-,formation_receipt_fingerprint:text:notnull:-,source_reference_observation_sequence:bigint:notnull:-,asset_id:text:notnull:-,binance_symbol:text:notnull:-,okx_inst_id:text:notnull:-,observed_price:numeric:notnull:-,observed_at:timestamp with time zone:notnull:-,captured_at:timestamp with time zone:notnull:-,source_evidence_sha256:text:notnull:-,binding_sha256:text:notnull:-,created_at:timestamp with time zone:notnull:-' then
  ...
  if v_trigger_fn is null or not exists (
    select 1
      from pg_catalog.pg_proc p
     where p.oid = v_trigger_fn and p.prorettype = 'pg_catalog.trigger'::regtype and p.prosecdef
  ) or not exists (
    select 1
