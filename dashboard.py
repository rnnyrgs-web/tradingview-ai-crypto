...
def _rows_for_dashboard(view, limit_per_horizon=20):
    """Read only real persisted opportunity rows; never invent dashboard trades."""
...
def _calibration_metrics(row):
    ...
    if not isinstance(calibration, dict):
        return {"accuracy":"LEARNING","floor":"—","n":"N=0/30","samples":0,"minimum_samples":30,"ready":False}
    try:
    ...
    except (TypeError,ValueError): minimum=30
    ready=bool(calibration.get("ready")); precision=calibration.get("empirical_precision") if ready else None; floor=calibration.get("precision_95pct_lower") if ready else None
    try: accuracy_text=f"{float(precision)*100.0:.0f}%" if precision is not None else "LEARNING"
    except (TypeError,ValueError): accuracy_text="LEARNING"
    try: floor_text=f"{float(floor)*100.0:.0f}%" if floor is not None else "—"
    except (TypeError,ValueError): floor_text="—"
    return {"accuracy":accuracy_text,"floor":floor_text,"n":f"N={samples}" if ready else f"N={samples}/{minimum}","samples":samples,"minimum_samples":minimum,"ready":ready}
...
def login_page(error=""):
    ...
    err=f'<div class="error">{html.escape(error)}</div>' if error else ""; disabled=" disabled" if not DASHBOARD_SECRET else ""
    return HTMLResponse(f"""<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Trading Research Command Center</title><style>body{{font-family:Arial,sans-serif;background:#0b0f14;color:#edf2f7;display:flex;min-height:100vh;align-items:center;justify-content:center;margin:0}}.box{{width:min(440px,90vw);background:#121821;border:1px solid #263240;border-radius:16px;padding:28px}}input,button{{box-sizing:border-box;width:100%;padding:13px;border-radius:10px;border:1px solid #344254;font-size:16px}}input{{background:#0b0f14;color:white;margin:12px 0}}button{{background:#edf2f7;color:#0b0f14;font-weight:bold;cursor:pointer}}button:disabled{{opacity:.45;cursor:not-allowed}}small{{color:#93a4b8}}.error{{background:#4a1c1c;padding:10px;border-radius:8px;margin:10px 0}}</style></head><body><div class="box"><h2>Trading Research Command Center</h2><p>Private owner dashboard for profitable-strategy research, 90-day 2x+ candidates, Money Intelligence, and the autonomous development fleet.</p>{err}<form method="post" action="/dashboard/login"><input name="secret" type="password" autocomplete="current-password" placeholder="Private dashboard password" required{disabled}><button{disabled}>Unlock dashboard</button></form><p><small>The password is submitted only to your Render service over HTTPS and replaced by a signed HttpOnly session cookie. Do not paste it into chat.</small></p></div></body></html>""")


async def handle_login(request: Request):
    if not DASHBOARD_SECRET: return login_page()
    raw=(await request.body()).decode("utf-8",errors="replace"); form=__import__("urllib.parse",fromlist=["parse_qs"]).parse_qs(raw,keep_blank_values=True); supplied=(form.get("secret") or [""])[0]
    if not hmac.compare_digest(supplied,DASHBOARD_SECRET): return login_page("Incorrect password.")
    expires=int(time.time())+SESSION_TTL_SECONDS; response=RedirectResponse("/dashboard?horizon=all",status_code=303)
...
def _nav(view):
    labels=(("all","ALL"),)+tuple((h,h) for h in DASHBOARD_HORIZONS)
    return "".join(f'<a class="{"active" if view==value else ""}" href="/dashboard?horizon={value}">{label}</a>' for value,label in labels)
...
def dashboard_page(request: Request,horizon="all"):
    ...
    view=_dashboard_view(horizon); rows=_rows_for_dashboard(view,limit_per_horizon=20); table_rows=[]
    for rank,row in enumerate(rows,1):
        lo,hi=_entry_zone(row); label=_trade_label(row); cls=label.lower(); score=float(row.get("evidence_score") or 0); rr=float(row.get("risk_reward") or 0); calibration=_calibration_metrics(row)
        stop_pct=_signed_pct(row,"stop_loss"); t1_pct=_signed_pct(row,"target_1"); t2_pct=_signed_pct(row,"target_2"); symbol=html.escape(str(row.get("symbol") or "")); signal_id=int(row.get("id") or 0); detail=f"/dashboard/signal/{signal_id}"; accuracy_cls="accuracy-ready" if calibration["ready"] else "accuracy-learning"
        table_rows.append(f"""<tr onclick="location.href='{detail}'"><td class="rank">{rank}</td><td class="symbol">{symbol}</td><td><span class="pill {cls}">{label}</span></td><td><span class="duration">{html.escape(_duration(row))}</span></td><td class="num {accuracy_cls}">{calibration['accuracy']}<small>forward</small></td><td class="num">{calibration['floor']}<small>95% floor</small></td><td class="num">{calibration['n']}<small>independent</small></td><td class="num">{_price(lo)}–{_price(hi)}</td><td class="num stop">{_price(row.get('stop_loss'))}<small>{_pct_text(stop_pct)}</small></td><td class="num target">{_price(row.get('target_1'))}<small>{_pct_text(t1_pct)}</small></td><td class="num target">{_price(row.get('target_2'))}<small>{_pct_text(t2_pct)}</small></td><td class="num">{rr:.2f}</td><td class="num">{score:.0f}<small>strength</small></td><td><a class="tv" href="{detail}">VIEW CHART</a></td></tr>""")
    empty="" if table_rows else '<div class="empty">No recent candidates for this view yet. The engine will not invent trades to fill the list.</div>'
    table="" if not table_rows else f"""<div class="tablewrap"><table><thead><tr><th>#</th><th>CRYPTO</th><th>SIGNAL</th><th>DURATION</th><th>ACCURACY</th><th>95% FLOOR</th><th>SAMPLE</th><th>ENTRY AREA</th><th>STOP LOSS</th><th>EXPECTED T1</th><th>EXPECTED T2</th><th>R:R</th><th>EVIDENCE</th><th>CHART</th></tr></thead><tbody>{''.join(table_rows)}</tbody></table></div>"""
    return HTMLResponse(f"""<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="refresh" content="60"><title>Crypto Signals</title><style>*{{box-sizing:border-box}}body{{margin:0;background:#081018;color:#eaf1f8;font-family:Arial,sans-serif}}.wrap{{max-width:1800px;margin:auto;padding:18px}}header{{display:flex;justify-content:space-between;gap:16px;align-items:end;flex-wrap:wrap}}h1{{margin:0 0 4px;font-size:24px}}.sub{{color:#91a4b7;font-size:13px}}nav{{display:flex;gap:6px;flex-wrap:wrap}}nav a{{color:#eaf1f8;text-decoration:none;border:1px solid #32465a;border-radius:8px;padding:8px 11px;display:inline-block}}nav a.active{{background:#eaf1f8;color:#081018}}.note{{background:#0d1823;border-left:3px solid #405a72;padding:9px 12px;margin:12px 0;color:#aebdca;font-size:12px}}.tablewrap{{overflow-x:auto;border:1px solid #213244;border-radius:10px;background:#0c1620}}table{{width:100%;border-collapse:collapse;min-width:1480px}}thead{{background:#111c27;position:sticky;top:0}}th{{font-size:10px;letter-spacing:.04em;color:#7f94a8;text-align:left;padding:9px 8px;border-bottom:1px solid #26394c;white-space:nowrap}}td{{padding:8px;border-bottom:1px solid #172635;font-size:12px;white-space:nowrap;vertical-align:middle}}tbody tr{{cursor:pointer}}tbody tr:hover{{background:#101d29}}.rank{{color:#60778d;width:34px}}.symbol{{color:#f0f6fb;font-weight:800;font-size:13px}}.duration{{font-weight:800;color:#b9d3ea;background:#162737;border:1px solid #29445c;border-radius:999px;padding:4px 7px}}.num{{font-variant-numeric:tabular-nums}}td small{{display:block;color:#8296a9;font-size:10px;margin-top:2px}}.pill{{font-weight:800;padding:4px 7px;border-radius:999px;font-size:10px}}.buy{{background:#123c2c;color:#9ee6c2}}.sell{{background:#482020;color:#ffb2b2}}.wait{{background:#403815;color:#eadb93}}.stop small{{color:#e0a0a0}}.target small{{color:#99d6b5}}.accuracy-ready{{font-weight:900;color:#9ee6c2}}.accuracy-learning{{font-weight:800;color:#eadb93}}.tv{{background:#2962ff;color:white;text-decoration:none;font-weight:800;border-radius:7px;padding:7px 9px;display:inline-block}}.empty{{padding:24px;background:#111c27;border-radius:10px;margin-top:14px;color:#aebdca;line-height:1.5}}@media(max-width:720px){{.wrap{{padding:10px}}h1{{font-size:20px}}th,td{{padding:7px 6px}}}}</style></head><body><div class="wrap"><header><div><h1>Crypto signals</h1><div class="sub">All trade horizons in one dashboard: 6h · 12h · 24h · 48h · 72h · 7d. Refreshes every 60 seconds.</div></div><nav>{_nav(view)}</nav></header><div class="note"><b>ALL</b> combines every persisted opportunity horizon. <b>Accuracy</b> is empirical forward hit rate from genuinely resolved, non-overlapping comparable forecasts. <b>95% Floor</b> is the conservative Wilson lower bound. <b>N</b> is independent forecasts. <b>Evidence</b> is signal strength, not probability. New horizons remain WAIT/LEARNING until their own evidence passes the existing gates.</div>{table}{empty}</div></body></html>""")


def signal_chart_data(request: Request,signal_id:int):
    if not _authorized(request): return JSONResponse({"ok":False,"error":"unauthorized"},status_code=401)
    row=fetch_signal_by_id(signal_id)
    if not row: return JSONResponse({"ok":False,"error":"signal_not_found"},status_code=404)
    symbol=str(row.get("symbol") or "BTC-USDT"); duration=_duration(row); bar="4H" if duration in {"48h","72h","7d"} else "1H"; candles=get_candles(symbol,bar=bar,limit=240)
