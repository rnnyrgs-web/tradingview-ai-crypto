...
def dashboard_page(request: Request, horizon="24h"):
    ...
    for rank, row in enumerate(rows, 1):
        ...
        overlay = f"/dashboard/signal/{signal_id}/tradingview-overlay"
        table_rows.append(f"""
<tr>
...
<td><a class="tv" target="_blank" rel="noopener" href="{html.escape(_tv_url(row))}">Chart ↗</a><a class="overlay" href="{overlay}">Overlay</a></td>
</tr>""")
...
<th>#</th><th>CRYPTO</th><th>SIGNAL</th><th>DURATION</th><th>ENTRY AREA</th><th>STOP LOSS</th><th>EXPECTED T1</th><th>EXPECTED T2</th><th>R:R</th><th>EVIDENCE</th><th>TRADINGVIEW</th>
...
*{{box-sizing:border-box}}body{{margin:0;background:#081018;color:#eaf1f8;font-family:Arial,sans-serif}}.wrap{{max-width:1600px;margin:auto;padding:18px}}header{{display:flex;justify-content:space-between;gap:16px;align-items:end;flex-wrap:wrap}}h1{{margin:0 0 4px;font-size:24px}}.sub{{color:#91a4b7;font-size:13px}}nav a{{color:#eaf1f8;text-decoration:none;border:1px solid #32465a;border-radius:8px;padding:8px 11px;margin-left:6px;display:inline-block}}nav a.active{{background:#eaf1f8;color:#081018}}.note{{background:#0d1823;border-left:3px solid #405a72;padding:9px 12px;margin:12px 0;color:#aebdca;font-size:12px}}.tablewrap{{overflow-x:auto;border:1px solid #213244;border-radius:10px;background:#0c1620}}table{{width:100%;border-collapse:collapse;min-width:1180px}}thead{{background:#111c27;position:sticky;top:0}}th{{font-size:10px;letter-spacing:.04em;color:#7f94a8;text-align:left;padding:9px 8px;border-bottom:1px solid #26394c;white-space:nowrap}}td{{padding:8px;border-bottom:1px solid #172635;font-size:12px;white-space:nowrap;vertical-align:middle}}tbody tr:hover{{background:#101d29}}.rank{{color:#60778d;width:34px}}.symbol a{{color:#f0f6fb;font-weight:800;text-decoration:none;font-size:13px}}.num{{font-variant-numeric:tabular-nums}}td small{{display:block;color:#8296a9;font-size:10px;margin-top:2px}}.pill{{font-weight:800;padding:4px 7px;border-radius:999px;font-size:10px}}.buy{{background:#123c2c;color:#9ee6c2}}.sell{{background:#482020;color:#ffb2b2}}.wait{{background:#403815;color:#eadb93}}.stop small{{color:#e0a0a0}}.target small{{color:#99d6b5}}.tv,.overlay{{text-decoration:none;font-weight:800;border-radius:7px;padding:7px 9px;display:inline-block;margin-right:5px}}.tv{{background:#2962ff;color:white}}.overlay{{background:#26394c;color:white}}.empty{{padding:24px;background:#111c27;border-radius:10px;margin-top:14px}}@media(max-width:720px){{.wrap{{padding:10px}}h1{{font-size:20px}}th,td{{padding:7px 6px}}}}
...
<div class="note">BUY/SELL appears only for validated engine TRADE candidates; otherwise WAIT. Percent values under stop/targets show distance from modeled entry. <b>Chart</b> opens the correct TradingView market/timeframe. <b>Overlay</b> generates the exact Pine overlay for that signal so BUY/SELL, entry, stop zone, targets, expected move %, duration, R:R and evidence can be drawn directly on TradingView.</div>{table}{empty}</div></body></html>""")
...
<p>{html.escape(str(row.get('reasoning') or ''))}</p><a class="tv" target="_blank" rel="noopener" href="{html.escape(_tv_url(row))}">OPEN TRADINGVIEW ↗</a><a class="overlay" href="{overlay}">GENERATE TV OVERLAY</a><a class="back" href="/dashboard?horizon={html.escape(str(row.get('timeframe') or '24h'))}">Back to signal rows</a><p><small>TradingView does not allow external sites to inject arbitrary Pine scripts or drawing objects through a normal chart URL. The overlay button therefore generates the exact Pine v6 script for this signal; paste it into TradingView Pine Editor and select Add to chart.</small></p></div></div></body></html>""")
